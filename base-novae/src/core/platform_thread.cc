                                  ...
                                  auto const dst_exc_base, auto const ec_sel)
{
	...
	auto const src_exc_base = pager.exc_pt_sel_client();
...
void Platform_thread::start(void *ip, void *sp)
{
	...
	if (!main_thread()) {
		...
		if (worker()) {
			...
			revoke(core_obj_sel, Obj_crd(pager.exc_pt_sel_client() + PT_SEL_STARTUP, 0));
		}
...
Trace::Execution_time Platform_thread::execution_time() const
{
	...
	if (!sc_created()) {
		...
		uint8_t res = Novae::ec_time(_sel_ec(), ec_time);
		if (res != Novae::NOVA_OK)
			warning("ec_time failed res=", res);
		return { ec_time, sc_time, Novae::Qpd::DEFAULT_QUANTUM, _priority };
	}

	uint8_t res = Novae::sc_ec_time(_sel_sc(), _sel_ec(), sc_time, ec_time);
	if (res != Novae::NOVA_OK)
