...
void Ipc_pager::set_reply_mapping(Mapping const &mapping)
{
	...
	_syscall_res = async_map(_pd_core, _pd_dst,
	                         nova_src_crd(mapping), nova_dst_crd(mapping));

	if (_syscall_res)
		error(__func__, " unimplemented ",
		      Hex(mapping.src_addr), " -> ",
		      Hex(mapping.dst_addr), "+ log=", Hex(mapping.size_log2), " result=", _syscall_res);
