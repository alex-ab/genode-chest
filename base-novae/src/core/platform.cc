	...
	return region_alloc().alloc_aligned(size + (guard_page ? get_page_size() : 0),
		...
		[&] (void *core_local_ptr) {
			...
			int res = map_local(_core_pd_sel, *__main_thread_utcb, phys_addr,
			                    ...
			                    Novae::Rights(true, true, false), true);
...
static void page_fault_handler()
{
		         ...
		         stack_area_virtual_size()))
	{
		...
		Novae::Utcb * utcb_fault = reinterpret_cast<Novae::Utcb *>(utcb_addr_f);
		...
		for (unsigned i = 0; i < last_items; i++) {
			Novae::Utcb::Item * item = utcb_fault->get_item(i);
			if (!item)
				break;

			Novae::Crd crd(item->crd);
			if (crd.is_null())
				continue;
...
//	_cpus = setup_affinity_space(hip);
	...
	log("Hello Genode@NOVAe!");

	error("stop for now here - to be continued");
