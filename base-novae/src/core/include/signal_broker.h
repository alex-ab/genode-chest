...
class Core::Signal_broker
{
	...
	public:
		...
		alloc_context(Capability<Signal_source>, unsigned long imprint)
		{
			...
			uint8_t res = Novae::create_si(cap.local_name(),
		...
		void free_context(Signal_context_capability context_cap)
		{
			...
			Novae::revoke(Novae::Obj_crd(context_cap.local_name(), 0));
		...
		{
			/*
			 * On Novae, signals are submitted directly to the kernel, not
