...
inline bool map_phys_to_core(Genode::addr_t const  phys_addr,
                             ...
                             Novae ::Rights const &permission)
{
	auto const &hip            = kernel_hip();
	auto const  kernel_mem_sel = hip.sel_num - 8; /* cap of kernel host space */
	auto const  core_mem_sel   = hip.sel_num - 9; /* cap of root   host space */
                        ...
                        Novae::Rights  const rwx)
{
	...
	unmap_local(core_mem_sel, start, num_pages, rwx);
}
