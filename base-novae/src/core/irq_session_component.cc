...
static bool irq_ctrl(addr_t irq_sel, addr_t &msi_addr, addr_t &msi_data,
                     Novae::Gsi_flags, addr_t bdf)
{
	/* assign IRQ to CPU && request msi data to be used by driver */
	uint8_t res = Novae::assign_int(irq_sel, kernel_hip().cpu_bsp, bdf,
	                                msi_addr, msi_data);
...
static bool associate_gsi(addr_t irq_sel, Signal_context_capability,
                          Novae::Gsi_flags gsi_flags)
{
	addr_t dummy1 = 0, dummy2 = 0;

	return irq_ctrl(irq_sel, dummy1, dummy2, gsi_flags, 0);
}
...
static void deassociate(addr_t irq_sel)
{
	...
	if (!irq_ctrl(irq_sel, dummy1, dummy2, Novae::Gsi_flags(), 0))
...
void Irq_object::sigh(Signal_context_capability cap)
{
	...
	bool ok = false;
	if (_bdf < 0x10000 || (_msi_addr && _msi_data))
...
void Irq_object::ack_irq()
{
	...
	if (res != Novae::NOVA_OK)
		error("Unmasking IRQ failed ", res);
}


void Irq_object::start(unsigned irq, addr_t const bdf, Irq_args const &irq_args)
{
	...
	if (irq_args.type() == Irq_session::TYPE_LEGACY)
		...
		ok = associate_msi(irq_sel(), bdf, _msi_addr, _msi_data);
	...
	_irq_number(~0U), _irq_alloc(irqs), _irq_object()
{
	...
	_irq_object.start(_irq_number, bdf, irq_args);
}
	                           ...
	                           private Core::Trace::Source
	{
		...
		Info trace_source_info() const override
		{
			...
			return { Session_label("core"), thread.name(),
			         Trace::Execution_time(ec_time, 0), thread.affinity() };
		}
