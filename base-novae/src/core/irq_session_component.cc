...
static bool irq_ctrl(addr_t irq_sel, addr_t &msi_addr, addr_t &msi_data,
                     addr_t sig_sel, Novae::Gsi_flags flags, addr_t virt_addr)
{
	/* assign IRQ to CPU && request msi data to be used by driver */
	uint8_t res = Novae::assign_gsi(irq_sel, virt_addr, boot_cpu(),
...
static void deassociate(addr_t irq_sel)
{
	...
	if (!irq_ctrl(irq_sel, dummy1, dummy2, irq_sel, Novae::Gsi_flags(), 0))
...
{
	if (!phys_mem)
		return irq_ctrl(irq_sel, msi_addr, msi_data, sig_cap.local_name(), Novae::Gsi_flags(), 0);
	...
	return platform().region_alloc().alloc_aligned(4096, 12).convert<bool>(
		...
		[&] (void *virt_ptr) {
			...
			Novae::Mem_crd phys_crd(phys_mem  >> 12, 0, Rights(true, false, false));
			Novae::Mem_crd virt_crd(virt_addr >> 12, 0, Rights(true, false, false));
			...
			bool res = irq_ctrl(irq_sel, msi_addr, msi_data, sig_cap.local_name(), Novae::Gsi_flags(), virt_addr);

			unmap_local(Novae::Mem_crd(virt_addr >> 12, 0, Rights(true, true, true)));
			platform().region_alloc().free(virt_ptr, 4096);
...
{
	if (Novae::NOVA_OK != Novae::sm_ctrl(irq_sel(), Novae::SEMAPHORE_DOWN))
...
Irq_object::~Irq_object()
{
	...
	/* revoke IRQ SM */
	Novae::revoke(Novae::Obj_crd(_kernel_caps, KERNEL_CAP_COUNT_LOG2));
