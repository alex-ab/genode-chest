...
void Rpc_entrypoint::_dissolve(Rpc_object_base *obj)
{
	...
	Novae::revoke(Novae::Obj_crd(obj->cap().local_name(), 0), true);
...
static void reply(Novae::Utcb &utcb, Rpc_exception_code exc, Msgbuf_base &snd_msg)
{
	copy_msgbuf_to_utcb(utcb, snd_msg, exc.value);

	Novae::reply(Thread::myself()->stack_top());
}
...
void Rpc_entrypoint::_activation_entry()
{
	...
	Novae::Utcb     &utcb = *(Novae::Utcb *)Thread::myself()->utcb();
	...
	if (!rcv_window.prepare_rcv_window(*(Novae::Utcb *)ep.utcb()))
		warning("out of capability selectors for handling server requests");
	...
	_pd_session(*pd_session)
{
	...
	if (!rcv_window.prepare_rcv_window(*(Novae::Utcb *)&_stack->utcb()))
