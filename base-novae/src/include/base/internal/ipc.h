 ...
 * than 0.
 */
static inline Novae::mword_t copy_utcb_to_msgbuf(Novae::Utcb             &utcb,
 ...
 */
static inline bool copy_msgbuf_to_utcb(Novae::Utcb               &utcb,
                                       Genode::Msgbuf_base const &snd_msg,
                                       Novae::mword_t             protocol_value)
{
	using namespace Genode;
	using namespace Novae;
	...
	for (unsigned i = 0; i < snd_msg.used_caps(); i++) {
		...
		Novae::Crd const crd = Capability_space::crd(cap);
