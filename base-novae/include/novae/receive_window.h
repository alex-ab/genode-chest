...
struct Genode::Receive_window
{
	...
	public:
		 ...
		 */
		bool prepare_rcv_window(Novae::Utcb &utcb,
		 ...
		 */
		void post_ipc(Novae::Utcb &utcb, addr_t const rcv_window = INVALID_INDEX)
		{
			...
			for (unsigned i = 0; i < rcv_items; i++) {
				Novae::Utcb::Item * item = utcb.get_item(i);
				if (!item)
					break;

				Novae::Crd cap(item->crd);
