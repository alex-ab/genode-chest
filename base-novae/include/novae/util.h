                                 ...
                                 Genode::addr_t const sel, Genode::addr_t event)
{
	...
	Novae::Crd orig_crd = utcb->crd_rcv;

	/* request event-handler portal */
	utcb->crd_rcv = Novae::Obj_crd(sel, 0);
	utcb->msg()[0]  = event;
	utcb->set_msg_word(1);

	Genode::uint8_t res = Novae::call(cap);
                                   ...
                                   Genode::addr_t const sel)
{
	...
	Genode::uint8_t res = Novae::NOVA_OK;
	enum {
